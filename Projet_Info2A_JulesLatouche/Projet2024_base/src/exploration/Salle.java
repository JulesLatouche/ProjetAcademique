...
{    
    ...
    */
    ...
    //Cette méthode assure la création d'un accès bidirectionnel entre la salle actuelle 
    //et sa salle voisine dans la direction spécifiée.
    public void setAcces(Direction d)
    {
         // Vérifie si l'accès dans cette direction est déjà présent
        if (!acces.contains(d)) {
            // Obtient la salle voisine dans la direction spécifiée
        Salle sVoisine = getVoisine(d);

        // Vérifie si la salle voisine existe
        if (sVoisine != null) {
            // Ajouter l'accès dans cette direction
            acces.add(d);
            // Ajoute l'accès inverse à la salle voisine
            Direction inverse = d.getInverse();
            sVoisine.acces.add(inverse);
        }
    }
    ...
    public void entree(Joueur j)
    {
        ...
        //verif si la methode prend fin et tourne pas en boucle 
        if(j.getPosition()==position && !isVide()){
            this.objet.interaction(j);
        }
        else{
